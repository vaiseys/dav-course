...
library(gapminder)             # we need this for data
...
gm2007 <- gm2007 |> 
  mutate(lifeExpMonths = lifeExp * 12,
         zYears = (lifeExp - mean(lifeExp)) / sd(lifeExp),
         zMonths = (lifeExpMonths - mean(lifeExpMonths)) / sd(lifeExpMonths) )
```

It doesn't matter if it's years or months, the z-score (standardized variable) is the same!
...
ggplot(d2,
  ...
  geom_smooth(method = "lm",
              se = FALSE,
              linetype = "solid") +
  facet_wrap(~home_away)
```

Let's try this with gapminder
...
ggplot(gm2007,
  ...
  labs(title = "Life Expectancy by GDP for 142 Countries",
       x = "Base 2 log of GDP per capita",
       y = "Life Expectancy at Birth",
       caption = "Source: 2007 Gapminder data")
