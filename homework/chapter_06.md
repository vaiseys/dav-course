...
# You should choose whatever theme works best for you.
# If you have the latest version of ggplot2, you should
# check out the new `ink`, `paper`, and `accent` arguments.
theme_set(theme_light())

# Read in the data 
url <- "https://raw.githubusercontent.com/vaiseys/dav-course/refs/heads/main/Data/gender_employment.csv"
gender_employment <- read_csv(url)
...
> Hint: You can find more information about this dataset [here](https://github.com/rfordatascience/tidytuesday/tree/main/data/2019/2019-03-05)
...
gender_employment |> 
  ...
  annotate("text", x = 2013.5, y = 102, label = "line of equality") +
  labs(title = "Women's earnings with respect to men's", 
       y = "% of Men's Income", 
       x = "Year")
```

<img src="nico_plot_chapter_6.png" width="70%"/>
...
"Chief Executives" belongs to the category "Management, Business, and Financial". Thus, this observation has a 0 for all other dummy variables for category membership and those betas will play no role in estimating our value. Wait, but in your model output there's no coefficient for "Management, Business, and Financial". That's because that's our reference category; it is baked into beta 0, the intercept. This is also why I said that adding a categorical variable of 8 categories implies adding 7 coefficients. You get the extra one for free because that's your new reference category. The betas then will tell you how much categories differ from your reference group.

Okay, that was a lot. Now you do some interpretation. Using the coefficients from your model, **calculate the wage percentage of male income for Sales and Office occupations in 2015.**

Now **calculate the wage percentage of male income for Service occupations in 2016.**

> Note: Notice that you could have calculated these values for any given year outside of the data (e.g., 2028, 3001). We call that *extrapolating*.

## Question 2

The model above assumes parallel trends. This means that we are telling the model to draw a line with the same slope for each major category, but we let the lines start at different points. The only thing that changes across major categories then is the intercept of the lines.
...
What does this mean? Occupation 2, in our case, is "Computer, Engineering, and Science". For this occupation, only four betas are contribute to the prediction: beta 0, beta 1, beta 2, and beta 9. All the others are multiplied by zero (\~they are "switched off").

So, if we want to calculate our estimate for the "Computer, Engineering, and Science" in 2016, we would do the following:
...
## year                  1.000                 0.0240           0.0050
## wage_percent_of_male  0.024                 1.0000           0.1115
## percent_female        0.005                 0.1115           1.0000
```
...
> Tip: Most real datasets have a tendency toward frustrating our expectations. It's OK if you do not see much happening.
...
Okay, let's now compare the R squared for `simple_fit` and `multiple_fit`. To do this we are going to use `glance()` from the `broom` package. Run `glance()` on `simple_fit`. This should give you a lot of information, including the R squared. You can save that output doing something like this.
